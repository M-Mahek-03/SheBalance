...
async function loadBulkOrders() {
    ...
    try {
        ...
        console.log('🔍 Fetching orders from http://localhost:5001/api/orders');
...
async function updateOrderProgress(orderId) {
    ...
    try {
        const response = await fetch(`http://localhost:5001/api/orders/${orderId}/progress`, {
            method: 'PUT',
            headers: {
...
async function initiateVoiceCall(orderId) {
    try {
        ...
        const response = await fetch('http://localhost:5001/api/voice-call/initiate', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('shebalance_token')}`
...
async function checkVoiceCallStatus(orderId) {
    try {
        const response = await fetch(`http://localhost:5001/api/voice-call/status/${orderId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('shebalance_token')}`
            }
        });
...
async function saveBalanceData() {
    ...
    try {
        const token = localStorage.getItem('shebalance_token');
        if (token) {
            const response = await fetch('http://localhost:5001/api/users/balance', {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
